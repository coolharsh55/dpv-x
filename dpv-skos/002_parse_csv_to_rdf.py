...
# SKOS FILES are written to EXPORT_DPV_MODULE_PATH
########################################

IMPORT_CSV_PATH = '../documentation-generator/vocab_csv'
EXPORT_DPV_PATH = './dpv'
...
def add_common_triples_for_all_terms(term, graph):
    ...
    # rdfs:label
    graph.add((BASE[f'{term.term}'], SKOS.prefLabel, Literal(term.rdfs_label, lang='en')))
    # dct:description
        ...
        if cls.rdfs_subclassof:
            ...
            for parent in parents:
                if parent.startswith('http'):
                    graph.add((BASE[f'{cls.term}'], SKOS.broader, URIRef(parent)))
                    graph.add((URIRef(parent), SKOS.narrower, BASE[f'{cls.term}']))
                elif ':' in parent:
                    # assuming something like rdfs:Resource
                    ...
                    graph.add((BASE[f'{cls.term}'], SKOS.broader, Literal(parent, datatype=XSD.string)))
                    graph.add((Literal(parent, datatype=XSD.string), SKOS.narrower, BASE[f'{cls.term}']))
...
graph = Graph()
graph.load('dpv-skos-metadata.ttl', format='turtle')
DPV_GRAPH += graph
...
graph = Graph()
graph.load('dpv-gdpr-skos-metadata.ttl', format='turtle')
DPV_GDPR_GRAPH += graph
